...
public class VendorOrdersToDeliverFragment extends Fragment {
    @Nullable

    DatabaseReference databaseReference;
    ArrayList<String> order_products_id;
    ArrayList<String> order_product_vendor_no;
    ArrayList<String> order_product_vendor_type;
    ArrayList<String> order_product_total_price;
    ArrayList<String> order_product_single_price;
    ArrayList<String> order_product_quantity;
    ArrayList<String> order_date;
    ArrayList<String> order_time;
    ArrayList<String> order_status;
    ArrayList<String> order_total_price;
    ArrayList<String> order_receiver_address;
    ArrayList<String> order_receiver_name;
    ArrayList<String> order_receiver_phn;
    ArrayList<String> order_customer_type;
    ...
    private static VendorProductsToDeliverCustomAdapter adapter;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        reference = FirebaseDatabase.getInstance().getReference();
        final Query query=reference.child("Orders").orderByChild("Vendor_Original_Phone").equalTo(User_Phone);
        query.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                dataModels=new ArrayList<>();
                adapter = new VendorProductsToDeliverCustomAdapter(dataModels, root.getContext());
...
//                        dataModels.add(new DataModel(customer_type,date,status,time,//total_price,p_id,quantity//,p_vendor_type,address,//name,receiver_phn));
                        fun(customer_type,  date,  status,  time,  total_price,  p_id,  single_price
                                ,  quantity,  vendor_no,  p_vendor_type,   address,   name,   receiver_phn,order_id); } } }
            @Override
    ...
    private void fun(String customer_type,String date,String status,String time,String total_price,String p_id,String single_price
            ,String quantity,String vendor_no,String p_vendor_type, String address, String name, String receiver_phn, String order_id){
...
//                            Toast.makeText(getActivity(), "TESTTTTT__"+customer_type, Toast.LENGTH_LONG).show();
...
//                            Toast.makeText(getActivity(), "TESTTTTT__"+customer_type, Toast.LENGTH_LONG).show();
    ...
    private void fetch_data_product_issue_function(DataSnapshot issue, String user_type,
         ...
         String p_vendor_type, String address, String name, String receiver_phn, String order_id){
//        Toast.makeText(getActivity(), ""+p_id, Toast.LENGTH_LONG).show();
...
//        Toast.makeText(getActivity(), "Resume", Toast.LENGTH_LONG).show();
        ...
        if (user_type.equals("member")){
            mainActivity.mTitle.setText(R.string.orders_to_deliver);
            mainActivity.layout_deliver_orders.setBackgroundColor(getResources().getColor(R.color.yellow));
            mainActivity.v4_upper.setBackgroundColor(getResources().getColor(R.color.yellow));
            mainActivity.v4_down.setBackgroundColor(getResources().getColor(R.color.yellow));
        }else{
            domainMainActivity.mTitle.setText(R.string.orders_to_deliver);
            domainMainActivity.layout_deliver_orders.setBackgroundColor(getResources().getColor(R.color.yellow));
            domainMainActivity.v4_upper.setBackgroundColor(getResources().getColor(R.color.yellow));
            domainMainActivity.v4_down.setBackgroundColor(getResources().getColor(R.color.yellow));
        }
    }
    @Override
    public void onPause() {
        if (user_type.equals("member")){
            mainActivity.layout_deliver_orders.getBackground().setAlpha(0);
            mainActivity.v4_upper.getBackground().setAlpha(0);
            mainActivity.v4_down.getBackground().setAlpha(0);
        }else{
            domainMainActivity.layout_deliver_orders.getBackground().setAlpha(0);
            domainMainActivity.v4_upper.getBackground().setAlpha(0);
            domainMainActivity.v4_down.getBackground().setAlpha(0);
        }
        super.onPause();
